...
// リセット後の待機時間 (ms) - 増加
#define RESET_DELAY_MS 300  // 50msから100msに増加
...
static int tlv493d_i2c_bus_recovery(const struct device *dev)
{
    ...
    // 1. General Reset (0x00) - データシート5.7.1に基づく最強のリセット
    LOG_DBG("Sending general reset during bus recovery");
    ret = tlv493d_send_general_reset(dev);
    if (ret < 0) {
        LOG_WRN("General reset failed during bus recovery (ret %d)", ret);
    }
    k_msleep(200); // General reset後の十分な安定化時間

    // 2. I2Cバスをリセットするためのシーケンス
    ...
    // 3. リカバリーフレーム(0xFF)を複数回送信し、センサーをリセット状態に誘導
...
static int tlv493d_initialize_sensor(const struct device *dev)
{
    ...
    k_msleep(1000); // 電源サイクル後の十分な待機時間
    
    while (retry_count < max_init_attempts) {
        LOG_DBG("Initialization attempt %d/%d", retry_count + 1, max_init_attempts);

        // // Step 1: General Reset (0x00) - データシート5.7.1に基づく根本的リセット
        // ret = tlv493d_send_general_reset(dev);
        // if (ret < 0) {
        //     LOG_WRN("General reset failed (ret %d)", ret);
        // } else {
        //     LOG_DBG("General reset sent successfully, waiting for sensor restart");
        // }
        // k_msleep(200); // General reset後の十分な待機時間
        
        // // Step 2: リカバリーフレームを複数回送信
        // for (int i = 0; i < 3; i++) {
        //     ret = tlv493d_send_recovery_frame(dev);
        //     if (ret < 0) {
